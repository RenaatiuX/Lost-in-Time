...
public class MesozoicChunkGenerator extends ChunkGenerator {

    public static final Codec<MesozoicChunkGenerator> CODEC = RecordCodecBuilder.create(
            (instance) -> instance.group(
                    ...
                    .apply(instance, instance.stable(MesozoicChunkGenerator::new)));
    ...
    public MesozoicChunkGenerator(BiomeProvider biomeProvider, long seed, Supplier<DimensionSettings> settings) {
        this(biomeProvider, biomeProvider, seed, settings);
    }

    private MesozoicChunkGenerator(BiomeProvider biomeProvider1, BiomeProvider biomeProvider2, long seed, Supplier<DimensionSettings> settings) {
    ...
    public ChunkGenerator func_230349_a_(long p_230349_1_) {
        return new MesozoicChunkGenerator(this.biomeProvider.getBiomeProvider(seed), seed, this.settings);
    }

    public boolean func_236088_a_(long p_236088_1_, RegistryKey<DimensionSettings> p_236088_3_) {
        return this.seed == p_236088_1_ && this.settings.get().func_242744_a(p_236088_3_);
    }
    ...
    public int getHeight(int x, int z, Heightmap.Type heightmapType) {
        return this.iterateNoiseColumn(x, z, (BlockState[]) null, heightmapType.getHeightLimitPredicate());
    }
    ...
    public List<MobSpawnInfo.Spawners> func_230353_a_(Biome biome, StructureManager manager, EntityClassification classification, BlockPos pos) {
        List<MobSpawnInfo.Spawners> spawns = net.minecraftforge.common.world.StructureSpawnManager.getStructureSpawns(manager, classification, pos);
        ...
        if (false) {//Forge: We handle these hardcoded cases above in StructureSpawnManager#getStructureSpawns, but allow for insideOnly to be changed and allow for creatures to be spawned in ones other than just the witch hut
            if (manager.getStructureStart(pos, true, Structure.SWAMP_HUT).isValid()) {
            ...
            if (classification == EntityClassification.MONSTER) {
                if (manager.getStructureStart(pos, false, Structure.PILLAGER_OUTPOST).isValid()) {
                    return Structure.PILLAGER_OUTPOST.getSpawnList();
                }

                if (manager.getStructureStart(pos, false, Structure.MONUMENT).isValid()) {
                    return Structure.MONUMENT.getSpawnList();
                }

                if (manager.getStructureStart(pos, true, Structure.FORTRESS).isValid()) {
